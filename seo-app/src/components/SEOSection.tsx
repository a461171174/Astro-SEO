  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] text-slate-400">预览在搜索引擎中的展示效果</span>
      </div>
      ...
      <div className="space-y-4 pt-4">
        <div className="space-y-2">
          <div className="flex justify-between">
            ...
            <span className="text-[11px] text-slate-400">{seoTitle?.length || 0}/70</span>
          </div>
          <input 
            type="text" 
            maxLength={70}
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="text-[11px] text-blue-600 hover:text-blue-700 font-bold flex items-center gap-1 disabled:opacity-50"
              >
                {isGenerating ? '生成中...' : 'AI 生成'}
                <ICONS.Palette className="w-2.5 h-2.5" />
              </button>
              <span className="text-[11px] text-slate-400">{seoDescription?.length || 0}/200</span>
            </div>
          </div>
          <textarea 
            maxLength={200}
